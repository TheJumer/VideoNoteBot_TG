...
```bash
git clone https://github.com/TheJumer/VideoNoteBot_TG.git
cd VideoNoteBot_TG
```
...
- Open `main.py`
- Replace the `TOKEN` variable with your actual Telegram Bot API token.

> **Note:** For production, it's recommended to use environment variables like `.env` to store your token securely.
...
**File Size:**  
The standard Telegram Bot API only allows bots to download files up to 20 MB. If a user sends a video larger than 20 MB, the bot will notify them about the limit.
